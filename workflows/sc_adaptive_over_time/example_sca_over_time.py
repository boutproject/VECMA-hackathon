...
def custom_moments_plot(var,results,filename,i):
    ...
    ax.fill_between(xvalues, results.describe(var, 'mean') -
                    results.describe(var, 'std'), results.describe(var, 'mean') +
                    results.describe(var, 'std'), label='std', alpha=0.2)
...
while sobols_error > 1e-3:
    ...
    moment_plot_filename = os.path.join(f"{campaign.campaign_dir}", "moments_centre"+str(i)+".png")
    moment_edge_plot_filename = os.path.join(f"{campaign.campaign_dir}", "moments_edge"+str(i)+".png")
    sobols_plot_filename = os.path.join(f"{campaign.campaign_dir}", "sobols_first_centre"+str(i)+".png")
    sobols_edge_plot_filename = os.path.join(f"{campaign.campaign_dir}", "sobols_first_edge"+str(i)+".png")
    ...
    plt.figure()
    results.plot_sobols_first("T_centre", ylabel="iteration"+str(i), xlabel=r"$t$", filename=sobols_plot_filename)
    plt.ylim(0,1)
    plt.savefig("sobols_centre"+str(i)+".png")
    plt.close()

    plt.figure()
    results.plot_sobols_first("T_edge", ylabel="iteration"+str(i), xlabel=r"$t$", filename=sobols_edge_plot_filename)
    plt.ylim(0,1)
...
ax.fill_between(xvalues, results.describe("T_centre", 'mean') -
                results.describe("T_centre", 'std'), results.describe("T_centre", 'mean') +
                results.describe("T_centre", 'std'), label='std', alpha=0.2)
ax.plot(xvalues, results.describe("T_centre", 'mean'), label='mean')
try:
    ax.plot(xvalues, results.describe("T_centre", '1%'), '--', label='1%', color='black')
    ax.plot(xvalues, results.describe("T_centre", '99%'), '--', label='99%', color='black')
except RuntimeError:
    pass
ax.grid(True)
